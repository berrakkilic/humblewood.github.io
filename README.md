...
One thing to know about free tiers: the filesystem often resets on redeploy, so the `data/table.db` database file and uploaded map/token images can get wiped when the service restarts or redeploys. Between sessions without redeploying, your table state is safe (the whole point of the SQLite database is surviving normal `Ctrl+C` / crash / restart cycles). For a table that survives *redeploys* too, the next step is a hosted database (e.g. Turso) and cloud image storage (Cloudflare R2 or S3) instead of local disk. Happy to wire that in whenever you're ready.
...
- Table state (map, tokens, playlist, sheets, roll log) is saved to a real SQLite database at `data/table.db` and reloads automatically on restart. Uploaded images still live on local disk, so they only persist as long as that disk does (see the deploy note above about free-tier redeploys).
