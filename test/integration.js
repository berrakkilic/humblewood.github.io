...
async function run() {
  ...
  for (const route of ['/map', '/characters', '/almanac', '/jukebox', '/library', '/dice']) {
    ...
    assert.match(html, /id="player-safety-btn"/);
    assert.match(html, /id="player-question-btn"/);
  ...
  assert.match(session0Source, /no sexual violence/);
  assert.match(session0Source, /critical initiative rolls/);
  ...
  const safetyNotificationPromise = once(dm.socket, 'notifications:update', notifications => notifications.some(notification => notification.type === 'safety'));
  const safetySubmittedPromise = once(playerOne.socket, 'safety:submitted');
  playerOne.socket.emit('safety:request');
  const [afterSafety, safetySubmitted] = await Promise.all([safetyNotificationPromise, safetySubmittedPromise]);
  const safetyNotification = afterSafety.find(notification => notification.type === 'safety');
  assert.equal(safetyNotification.anonymous, true);
  assert.equal(safetyNotification.senderName, null);
  assert.equal(safetySubmitted.breakMinutes, 15);

  const questionNotificationPromise = once(dm.socket, 'notifications:update', notifications => notifications.some(notification => notification.type === 'question' && notification.question === 'What should I bring?'));
  const questionSubmittedPromise = once(playerOne.socket, 'question:submitted');
  playerOne.socket.emit('question:submit', { question: 'What should I bring?', anonymous: false });
  const afterQuestion = await questionNotificationPromise;
  await questionSubmittedPromise;
  const questionNotification = afterQuestion.find(notification => notification.type === 'question' && notification.question === 'What should I bring?');
  assert.equal(questionNotification.senderName, 'Hazel');
  assert.equal(questionNotification.anonymous, false);
  const playerQuestionDenied = once(playerTwo.socket, 'action:denied', denial => /only the dungeon master/i.test(denial.message));
  playerTwo.socket.emit('notifications:clearAll');
  await playerQuestionDenied;
  pending = once(dm.socket, 'notifications:update', notifications => notifications.every(notification => notification.read));
  dm.socket.emit('notifications:markAllRead');
  const readNotifications = await pending;
  assert(readNotifications.length >= 2);
  pending = once(dm.socket, 'notifications:update', notifications => !notifications.some(notification => notification.id === safetyNotification.id));
  dm.socket.emit('notifications:clear', { id: safetyNotification.id });
  await pending;
